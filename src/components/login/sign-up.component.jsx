...
export const SignUp = ({ successAuth, setPage, showAlert }) => {
  ...
  const handleSubmit = (e) => {
    ...
    Firebase.signUp(email, password).then((response) => {
      if (response) {
        setTimeout(() => {
          successAuth();
        }, 1000);
  ...
  return (
    <form onSubmit={(e) => handleSubmit(e)} className={"card bg-secondary p-2 animate__slideInRight"}>
      <div className={"row justify-content-between"}>
        ...
        <div className={"col-12"}>
          <Actions text={"If you have an account, sign-in now"} handle={() => setPage()} />
        </div>
      </div>
    </form>
  );
